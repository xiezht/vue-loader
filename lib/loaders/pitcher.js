...
module.exports.pitch = function (remainingRequest) {
  console.log(`执行Pitcher Loader的 pitch 函数: ${remainingRequest}`)
  ...
  const genRequest = loaders => {
    ...
    // Normal 阶段是按 前置(pre)、普通(normal)、行内(inline)、后置(post) 的顺序调用loader的
    // Pitching 阶段则相反。
  ...
  // e.g. 比如 script 的模块经过 pitch 函数处理，得到
  ...
  // 忽略pre-loader的行内loader，从右往左，执行 vue-loader 再执行 babel-loader
  // vue-loader 其实就只执行了 selectBlock 函数，把对应 block 的代码传递给下一个 loader 处理（loaderContext.callback() / return ）
